...
public class ReindexFilter
{

    private static String fromESFilterIndexName;
    private static String toESFilterIndexName;
    private static String esIndexType;

    static {
        LocalPropertyFileHandler.getInstance().prepare( EnvConstants.getProfile() );

        fromESFilterIndexName = LocalPropertyFileHandler.getInstance().getProperty( "application", "fromESFilterIndexName" )
            .orElse( null );
        toESFilterIndexName = LocalPropertyFileHandler.getInstance().getProperty( "application", "toESFilterIndexName" )
            .orElse( null );
        esIndexType = LocalPropertyFileHandler.getInstance().getProperty( "application", "esIndexType" ).orElse( "docs" );
    }


    public static void filerReindex()
    {


        if ( fromESFilterIndexName == null || toESFilterIndexName == null ) {
            System.out.println( "Index name is null, Exiting" );
            System.exit( 0 );
        }
        final Scroll scroll = new Scroll( TimeValue.timeValueMinutes( 1L ) );
        SearchRequest searchRequest = new SearchRequest( fromESFilterIndexName );
        ...
        try {
            ...
            while ( hits != null && hits.length > 0 ) {
                for ( SearchHit hit : hits ) {
                    ...
                    try {
                        IndexRequest request = new IndexRequest( toESFilterIndexName, esIndexType,
